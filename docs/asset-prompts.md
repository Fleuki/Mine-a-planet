...
| Иконка руды | `ORE_ART[id]` → `drawOreGem()` | Прозрачный фон, 1:1. Свечение под камнем остаётся процедурным |

> ✅ **`PLANET_ART` теперь принимает развёртку, а не «лицо».** Раньше ветка
> `if (custom)` рисовала картинку статично и выбрасывала облака — планета
> замирала. Сейчас кастомная карта прокручивается тем же `_drawStrip`, что и
> процедурная, облака остаются. Поэтому генерируй **полосу 2.2:1**, круглое
> лицо планеты не подойдёт.
>
> Тонкость про бесшовность: процедурные полосы — шум, они тайлятся с периодом
> в половину ширины. Цельная карта так не может (была бы видна только её левая
> половина), поэтому для кастомной картинки период равен полной ширине —
> параметр `wrapDiv` в `_drawStrip`. Стык левого и правого края поедет через
> диск планеты, так что бесшовность обязательна.
...
var ORE_ART    = { dust: "assets/ores/dust.png" };       // ключ — id руды
```

Всё остальное уже работает: `preloadArt()` в `main.js` подхватит все три
таблицы, `getArt()` закэширует, рендер подставит. Чего нет в списке — рисуется
процедурно. **Отсутствующий файл не ломает игру** — `getArt` пометит его
`failed`, и рендер молча вернётся к процедурной отрисовке.
